...
class Config:
    ...
    # 颜色分割预处理配置
    ENABLE_COLOR_SEGMENTATION = True  # 是否启用颜色分割预处理
    COLOR_SEGMENTATION_DIR = "data/color_segmented"  # 预处理后的图片缓存目录
    GREEN_HSV_LOWER = [25,  50,  50]  # 绿色HSV下界 [H, S, V]
    GREEN_HSV_UPPER = [85, 255, 255]  # 绿色HSV上界 [H, S, V]
    USE_MORPHOLOGY = True  # 是否使用形态学操作优化分割
    FORCE_REPROCESS = False  # 是否强制重新处理（忽略缓存）
