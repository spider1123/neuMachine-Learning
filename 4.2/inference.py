...
from datasets import FoveaTestDataset, resize_keep_ratio
...
def run_inference(device, out_path="submission.csv", num_folds=5, tta_flip=False):
    ...
    for img_t, meta in test_loader:
        ...
        if p_vis < VIS_THRESH:
            ...
            # 根据阶段一坐标裁剪 patch
            # 注意：此处重新读取原 resize 图像，避免 DataLoader 中的归一化影响裁剪
            # 可以复用 datasets.resize_keep_ratio
            # 为简单起见，这里用当前 img_t 逆变换到 numpy（近似），也可在 Dataset 中返回原图路径
            img_np = (img_t.cpu().numpy()[0].transpose(1, 2, 0) * 255.0).astype(
                np.uint8
            )
            patch, x1, y1, w1p, h1p = crop_patch_from_coord(img_np, x_c, y_c)

            patch_t = (
                torch.from_numpy(patch).permute(2, 0, 1).float().unsqueeze(0) / 255.0
            ).to(device)
