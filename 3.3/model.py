...
class ResNet50MultiTask(nn.Module):
    ...
    辅助任务：关键点热图回归
    """
    def __init__(self, num_classes=6, num_landmarks=68, use_landmark=True):
        """
        Args:
            num_classes: 分类数量
...
def create_resnet50(num_classes=6, num_landmarks=68, use_landmark=True):
    """
    创建ResNet50多任务模型
    
    Args:
        num_classes: 分类数量
        num_landmarks: 关键点数量（默认68，与dlib标准一致）
...
if __name__ == '__main__':
    # 测试模型（带关键点）
    model = create_resnet50(num_classes=6, num_landmarks=68, use_landmark=True)
