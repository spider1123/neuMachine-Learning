...
class GrayImageDataset(Dataset):
    ...
    def __init__(self, image_paths, labels, transform=None, landmark_data=None, use_landmark=False):
        ...
        # 如果使用关键点监督且提供了自定义 transform，强制要求其带有 keypoint_params，
        # 以避免因为忘记配置 keypoints 而导致坐标体系错乱
        if self.use_landmark and self.transform is not None:
            if not hasattr(self.transform, 'keypoint_params'):
                raise ValueError(
                    "GrayImageDataset: 当 use_landmark=True 且自定义 transform 不为 None 时，"
                    "必须在 Albumentations.Compose 中设置 keypoint_params=KeypointParams(...)，"
                    "否则关键点增强会出错。"
                )
    ...
    def __getitem__(self, idx):
        ...
        # 注意：如果有关键点，需要在增强时同步变换关键点坐标（使用像素坐标体系）
        if self.transform:
            if landmarks is not None and len(landmarks) > 0:
                # 将关键点转换为Albumentations格式（像素坐标，基于原始48×48图像）
                # landmarks格式: [x1, y1, x2, y2, ...]
                keypoints = []
                for i in range(0, len(landmarks), 2):
                    if i + 1 < len(landmarks):
                        # 保持像素坐标（48×48），交给Albumentations根据几何变换自动缩放/平移
                        x = float(landmarks[i])
            ...
            if landmarks is not None:
                # 与上面流程保持一致：以像素坐标传入，由Albumentations处理
                keypoints = []
                for i in range(0, len(landmarks), 2):
                    if i + 1 < len(landmarks):
                        keypoints.append((float(landmarks[i]), float(landmarks[i + 1])))
                transformed = transform_basic(image=img, keypoints=keypoints)
