...
def train_epoch(model, train_loader, criterion, optimizer, device, scheduler=None,
                use_landmark=False, num_landmarks=68, use_amp=False, scaler=None):
    """训练一个epoch（支持多任务学习）"""
    model.train()
    running_loss = 0.0
...
@torch.no_grad()
def validate(model, val_loader, criterion, device, use_landmark=False, num_landmarks=68):
    """验证（支持多任务学习）"""
    model.eval()
    running_loss = 0.0
    running_cls_loss = 0.0
...
def main():
    ...
    if config.use_landmark:
        ...
        if len(landmark_data) > 0:
            ...
            # 同时关闭模型中的关键点分支，避免白算 heatmap
            if hasattr(model, 'use_landmark'):
                model.use_landmark = False
    ...
    # 创建数据加载器（固定划分，DataLoader 复用，每个 epoch 内部 shuffle）
    train_loader = DataLoader(
        train_dataset,
        batch_size=config.batch_size,
        shuffle=True,  # 每个epoch都会重新打乱
