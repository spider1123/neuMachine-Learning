...
def main():
    ...
    try:
        ...
        classifier = PlantClassifier(
            n_estimators=1000,      # 树的数量（增加到500轮以充分训练）
            max_depth=20,          # 树的最大深度
            learning_rate=0.01     # 学习率
        )
        
        # 训练模型（启用轻量数据增强，使用验证集进行 early stopping）
        # 注意：如果准确率仍然较低，可以尝试关闭增强进行对比测试：
        # use_augmentation=False  # 关闭增强查看 baseline
        print("开始训练模型（使用 early stopping 监控验证集性能）...")
        classifier.fit(
            train_paths_split, train_labels_split, label_map,
            val_image_paths=val_paths_split,
            val_labels=val_labels_split,
            use_augmentation=True,  # 使用轻量版增强（已在 model.py 中配置）
