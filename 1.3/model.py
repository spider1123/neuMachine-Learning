...
class PlantClassifier:
    ...
    def fit(self, image_paths:List[str], labels:List[int], label_map:Dict[str,int], 
        ...
        # 提取训练特征
        features = self.feature_extractor.extract_batch(image_paths, is_training=use_augmentation)
        ...
        if eval_set is not None:
            ...
            self.classifier.fit(features_scaled, labels, eval_set=eval_set)
            if verbose:
...
def _extract_features_safe(image_path: str, target_size: Tuple[int, int], is_training: bool = False) -> np.ndarray:
    ...
    if is_training:
        try:
            ...
            image = augment_image(image, target_size=target_size, is_training=True)
            augmentation_applied = True
        except ImportError:
            # 如果 augmentation 模块不存在，跳过增强
            pass
