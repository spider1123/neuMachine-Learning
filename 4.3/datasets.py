...
class FoveaStage2Dataset(Dataset):
    ...
    def _crop_patch(self, img_resized, x_res, y_res):
        ...
        # 加入随机扰动（jitter），模拟 Stage1 预测误差，缓解 exposure bias
        jitter = 16.0  # 可调参数，范围 8-32
        dx = random.uniform(-jitter, jitter)
        dy = random.uniform(-jitter, jitter)
        cx, cy = int(x_res + dx), int(y_res + dy)
